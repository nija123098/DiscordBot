...
public class MessageDeleteLogConfig extends AbstractConfig<Channel, Guild> {
    ...
    public void handle(DiscordMessageDelete delete){
        ...
        MessageMaker maker = new MessageMaker(channel).withColor(Color.GRAY).withAuthor(delete.getMessage().getAuthor()).append("Message deleted from ").appendRaw(delete.getAuthor().getDisplayName(delete.getGuild())).append(" by").appendRaw(" ").append("in ").appendRaw(delete.getChannel().mention()).appendRaw("\n" + delete.getMessage().getMentionCleanedContent());

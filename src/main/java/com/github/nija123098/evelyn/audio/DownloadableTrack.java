...
public abstract class DownloadableTrack extends Track {
    private static final List<String> TYPES = Stream.of(ReadConfig.AUDIO_FILE_TYPES.split(",")).map(String::trim).collect(Collectors.toList());
    ...
    public static List<Track> getDownloadedTracks(){
        File[] downloadedMusic = new File(ReadConfig.AUDIO_PATH).listFiles();
    ...
    public AudioTrack getAudioTrack(GuildAudioManager manager){
        ...
        if (playTimes >= ReadConfig.REQUIRED_PLAYS_TO_DOWNLOAD && ReadConfig.REQUIRED_PLAYS_TO_DOWNLOAD != -1) MusicDownloadService.queueDownload(playTimes == 0 ? 0 : (int) Math.log(playTimes), this, manager == null ? null : downloadableTrack -> manager.swap(this));
    ...
    public void manage(){
        super.manage();
        if (MusicDownloadService.isDownloaded(this) && ConfigHandler.getSetting(TrackTimeExpireConfig.class, this) + ReadConfig.TRACK_EXPIRATION_TIME < System.currentTimeMillis()){
            this.file().delete();
    ...
    private static final File AUDIO_CONTAINER = new File(ReadConfig.AUDIO_PATH);
    ...
    public File file(){// used when playing, and deleting, but at deletion it does not matter
        ...
        return new File(ReadConfig.AUDIO_PATH + this.getID() + "." + ConfigHandler.getSetting(TrackTypeConfig.class, this));
    }
    public String getPreferredType(){
        return ReadConfig.AUDIO_FORMAT;
    }
    @Override

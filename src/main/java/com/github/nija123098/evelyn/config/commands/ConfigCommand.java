...
public class ConfigCommand extends AbstractCommand {
    ...
    public <C extends Configurable> void command(@Argument(optional = true, info = "config/user/channel/role") C configurable, @Argument String s, User user, @Context(softFail = true) Guild guild, MessageMaker maker, @Context(softFail = true) Message message){
        if (configurable != null || s == null || s.isEmpty()){
            ...
            Stream.of(ConfigCategory.values()).filter(configCategory -> configCategory.getBotRole().hasRequiredRole(user, guild)).forEach(configCategory -> {
                if (configCategory.getConfigs().stream().filter(AbstractConfig::isNormalViewing).filter(abstractConfig -> abstractConfig.getConfigLevel() == finalConfigurable.getConfigLevel()).filter(abstractConfig -> abstractConfig.getBotRole().hasRequiredRole(user, guild)).peek(config -> maker.getNewFieldPart().withBoth(config.getName(), ConfigHandler.getExteriorSetting(config.getName(), finalConfigurable))).count() > 0){

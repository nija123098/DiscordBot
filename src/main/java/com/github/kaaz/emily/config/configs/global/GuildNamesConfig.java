 ...
 */
public class GuildNamesConfig extends AbstractConfig<Set<String>, Configurable.GlobalConfigurable> {
    ...
    public void handle(DiscordGuildJoin join){
        Set<String> set = ConfigHandler.getSetting(GuildNamesConfig.class, Configurable.GLOBAL);
        set.add(join.getGuild().getName());
        ConfigHandler.setSetting(GuildNamesConfig.class, Configurable.GLOBAL, set);
    }
    @EventListener
    ...
    public void handle(DiscordDataReload reload){
        ConfigHandler.setSetting(GuildNamesConfig.class, Configurable.GLOBAL, DiscordClient.getGuilds().stream().map(Guild::getName).collect(Collectors.toSet()));
    }
}

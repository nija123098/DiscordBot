...
public class GuildUserReputationConfig extends AbstractConfig<Integer, GuildUser> {
    private static final EmoticonHelper.Emoticon POSITIVE_REP = EmoticonHelper.getEmoticon("+1"), NEGATIVE_REP = EmoticonHelper.getEmoticon("-1");
    ...
    public void handle(DiscordReactionEvent event){
        ...
        int value = (event.getReaction().getEmoticon() == POSITIVE_REP ? 1 : event.getReaction().getEmoticon() == NEGATIVE_REP ? -1 : 0) * (event.addingReaction() ? 1 : -1);
        if (value == 0) return;

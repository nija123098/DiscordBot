...
public class PingUserCommand extends AbstractCommand {
    ...
    @Command
    public void command(@Convert User user, @Convert User u2, @Convert Channel channel, @Context MessageMaker maker, @Context String args){
        maker.appendContent(user.mention() + " ==> " + u2.mention() + "\nMessage:    " + args).withChannel(channel);
    }
}

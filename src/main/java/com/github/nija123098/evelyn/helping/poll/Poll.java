 ...
 */
public class Poll {
    private static final ScheduledExecutorService SCHEDULED_EXECUTOR_SERVICE = Executors.newSingleThreadScheduledExecutor(r -> ThreadHelper.getDemonThreadSingle(r, "Poll-Thread"));
    ...
    public Poll(String question, String[] options, long expiration, int maxVotes, Message origin) {
        ...
        if (expiration - System.currentTimeMillis() < MIN_POLL_DURATION)
            throw new ArgumentException("Polls must be open for at least one minute");
        if (expiration - System.currentTimeMillis() > MAX_POLL_DURATION)
    ...
    private void setUp() {
        SCHEDULED_EXECUTOR_SERVICE.schedule(this::onExpiration, this.expiration - System.currentTimeMillis(), TimeUnit.MILLISECONDS);

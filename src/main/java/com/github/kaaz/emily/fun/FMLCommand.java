...
public class FMLCommand extends AbstractCommand {
    ...
    private void getItems(){
        try {
            ...
            if (document != null) items.addAll(document.select("p.block a[href^=/article/]").stream().map(Element::text).map(String::trim).map(StringEscapeUtils::escapeHtml4).map(s -> s.length() > 2000 ? s.substring(0, 1999) : s).collect(Collectors.toSet()));
        } catch (IOException e) {
